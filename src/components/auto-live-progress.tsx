...
type ProgressItem = {
  ...
  manualResult: 'On Time' | 'Ahead' | 'Behind' | 'Just Complete' | null;
...
export default function AutoLiveProgressScreen() {
  ...
  useEffect(() => {
    ...
    for (const plan of taskPlans) {
      ...
      if (existing?.startedAt && (!existing.startMode || existing.startMode === 'manual')) {
        ...
        const status: TaskStatus = start !== null && currentMinute >= start ? 'In Progress' : 'Not Started';
        if (!existing.startMode || existing.status !== status) {
          next[index] = { ...existing, startMode: 'manual', status };
          changed = true;
        }
        continue;
      }
  ...
  function completeTask(plan: TeamTaskPlan) {
    ...
    const result = difference > 0 ? 'Ahead' : difference < 0 ? 'Behind' : 'On Time';
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.infoCard}>
          ...
          <Text style={styles.infoText}>No timer or Start button. Each employee becomes active automatically at the rostered shift start. Load tasks follow the approved Team Plan automatically after the load arrives. Use Adjust Start only when the real start was different.</Text>
        </View>
