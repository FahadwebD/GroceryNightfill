...
import {
  buildLiveHelpSuggestions,
  createLiveHelpAction,
  MANAGER_HELP_TOLERANCE_MINUTES,
  type LiveHelpAction,
  type LiveHelpSuggestion,
} from '../utils/liveHelpEngine';
import {
  buildEmployeePlans,
  buildTeamTaskPlans,
  dateToNightMinutes,
...
export default function LiveProgressHelpScreen() {
  ...
  const [helpActions, setHelpActions] = useState<LiveHelpAction[]>([]);
  const [loading, setLoading] = useState(true);
  const [now, setNow] = useState(Date.now());
  const [editingTask, setEditingTask] = useState<TeamTaskPlan | null>(null);
  const [startInput, setStartInput] = useState('');
  const [manualHelpTask, setManualHelpTask] = useState<string | null>(null);
  ...
  async function loadData() {
    try {
      ...
      setHelpActions(savedHelpActions || []);
      setNow(Date.now());
    } catch (error) {
  ...
  function effectiveStart(plan: TeamTaskPlan, item?: ProgressItem) {
    const saved = item?.startedAt
      ? dateToNightMinutes(item.startedAt)
      : null;
  ...
  function displayStatus(plan: TeamTaskPlan): TaskStatus {
    ...
    return currentMinute >= effectiveStart(plan, item)
      ? 'In Progress'
      : 'Not Started';
  }
  ...
  async function saveHelpActions(next: LiveHelpAction[]) {
    setHelpActions(next);
    await saveNightValue(
      NIGHTFILL_STORAGE.helpActions,
      dateKey,
  ...
  useEffect(() => {
    ...
    for (const plan of taskPlans) {
      ...
      if (
        existing?.startedAt &&
        (!existing.startMode || existing.startMode === 'manual')
      ) {
        const start = dateToNightMinutes(existing.startedAt);
        const desired: TaskStatus =
          start !== null && currentMinute >= start
            ? 'In Progress'
            : 'Not Started';
      ...
      const desiredAutoStart = nightMinuteToIso(
        nightfillDate,
        plan.plannedStartMinute
      );
      const desiredAutoStatus: TaskStatus =
        currentMinute >= plan.plannedStartMinute
          ? 'In Progress'
          : 'Not Started';

      if (existing?.startMode === 'auto') {
        if (
          existing.startedAt !== desiredAutoStart ||
          existing.status !== desiredAutoStatus
        ) {
          next[index] = {
            ...existing,
  ...
  const activeHelpActions = helpActions.filter(
  ...
  function openStartEditor(plan: TeamTaskPlan) {
    ...
    setEditingTask(plan);
    setStartInput(
      nightMinuteTo24Hour(effectiveStart(plan, item))
    );
  }
  ...
  async function saveManualStart() {
    ...
    if (minute === null) {
      Alert.alert(
        'Check Start Time',
        'Enter a valid 24-hour time, for example 20:15.'
      );
      return;
    }
    ...
    const next = existing
      ? progress.map((entry) =>
          entry.taskName === editingTask.taskName ? item : entry
        )
      : [...progress, item];
  ...
  async function useAutomaticStart(plan: TeamTaskPlan) {
    ...
    const item: ProgressItem = {
      ...
      status: complete
        ? 'Complete'
        : arrival?.arrived && currentMinute >= plan.plannedStartMinute
          ? 'In Progress'
          : 'Not Started',
      requiredMinutes: requiredMinutes(load, plan.taskName),
    ...
    const next = existing
      ? progress.map((entry) =>
          entry.taskName === plan.taskName ? item : entry
        )
      : [...progress, item];
  ...
  function completeTask(plan: TeamTaskPlan) {
    if (!arrival?.arrived) {
      Alert.alert(
        'Load Not Arrived',
        'Record the actual load arrival before completing load tasks.'
      );
      return;
    }
    ...
    Alert.alert(
      ...
      `Planned finish: ${formatNightMinute(
        plan.plannedFinishMinute
      )}\nComplete now: ${formatNightMinute(
        actualFinish
      )}\n\nResult: ${resultText}`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            const item: ProgressItem = {
              ...
              manualResult: result,
              manualDifferenceMinutes: Math.abs(
            ...
            if (difference > 0) {
              await appendAuditLog({
                category: 'Progress',
                action: 'Task finished early',
                details: `${plan.taskName} · ${Math.round(
                  difference
                )}m early · spare labour can be reallocated`,
              });
            }
          },
        },
  ...
  function taskPosition(plan: TeamTaskPlan) {
    ...
    if (status === 'Complete' && item?.completedAt) {
      ...
      if (difference < 0) {
        return `${formatMinutes(Math.abs(difference))} behind`;
      }
      return 'On time';
    }
    ...
    return `Auto starts ${formatNightMinute(
      effectiveStart(plan, item)
    )}`;
  }

  async function assignHelp(suggestion: LiveHelpSuggestion) {
    const action = createLiveHelpAction(
      suggestion,
      manualHelpTask === suggestion.toTaskName
        ? 'manual'
        : 'auto'
    );

    const next = [...helpActions, action];
    await saveHelpActions(next);

    await appendAuditLog({
      category: 'Progress',
      action: 'Live help assigned',
      details: `${suggestion.helperName} → ${suggestion.toTaskName} · ${suggestion.suggestedMinutes}m`,
    });

    setManualHelpTask(null);
  }

  async function finishHelp(action: LiveHelpAction) {
    const next = helpActions.map((item) =>
      item.id === action.id
        ? {
            ...item,
    ...
    await appendAuditLog({
      category: 'Progress',
      action: 'Live help completed',
      details: `${employeeName(
        employees,
        action.helperEmployeeId
      )} → ${action.toTaskName} · ${action.minutes}m`,
    });
  }

  async function cancelHelp(action: LiveHelpAction) {
    const next = helpActions.map((item) =>
      item.id === action.id
        ? {
            ...item,
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        <Text style={styles.muted}>Loading live progress...</Text>
      </View>
    );
  }

  const completed = taskPlans.filter(
    (plan) => displayStatus(plan) === 'Complete'
  ).length;
  const active = taskPlans.filter(
    (plan) => displayStatus(plan) === 'In Progress'
  ).length;
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <TouchableOpacity onPress={() => router.back()}>
          <Text style={styles.back}>‹ Team Plan</Text>
        </TouchableOpacity>
        <Text style={styles.title}>Live Progress</Text>
        <Text style={styles.headerSub}>
          {dayName} Nightfill · {dateKey}
        </Text>
      </View>
      ...
      >
        <View style={styles.infoCard}>
          <Text style={styles.infoTitle}>Live Help & Early-Finish Reallocation</Text>
          <Text style={styles.infoText}>
            Tasks start automatically. Finishing an aisle early creates spare shift capacity. If another aisle moves more than {MANAGER_HELP_TOLERANCE_MINUTES}m beyond plan, the app asks for Nightfill Manager help and suggests who can assist.
          </Text>
        </View>
        ...
        <View style={styles.row}>
          <View style={styles.statCard}>
            ...
            <Text style={styles.value}>{completed}/{taskPlans.length}</Text>
          </View>
          <View style={styles.statCard}>
          ...
          <View style={styles.statCard}>
            <Text style={styles.label}>Help Active</Text>
            <Text style={styles.value}>{activeHelpActions.length}</Text>
          </View>
        </View>

        {helpResult.helpNeededTasks.length > 0 ? (
          <View style={styles.managerHelpCard}>
            <Text style={styles.managerHelpTitle}>⚠️ Nightfill Manager Help Needed</Text>
            <Text style={styles.managerHelpText}>
              Small delays up to {MANAGER_HELP_TOLERANCE_MINUTES}m are tolerated. These tasks are now beyond that limit.
            </Text>
            {helpResult.helpNeededTasks.map((item) => (
              <Text key={item.taskName} style={styles.managerHelpItem}>
                {item.taskName}: {formatMinutes(item.beyondToleranceMinutes)} help recommended · {item.reason}
              </Text>
            ))}
          </View>
        ) : null}

        {helpResult.suggestions.length > 0 ? (
          <>
            <Text style={styles.sectionTitle}>Suggested Help</Text>
            {helpResult.suggestions.map((suggestion, index) => (
              <View
                key={`${suggestion.helperEmployeeId}-${suggestion.toTaskName}-${index}`}
                style={styles.helpSuggestionCard}
              >
                <View style={{ flex: 1 }}>
                  <Text style={styles.helpSuggestionTitle}>
                    {suggestion.helperName} → {suggestion.toTaskName}
                  </Text>
                  <Text style={styles.small}>
                    {suggestion.helperReason}
                  </Text>
                  <Text style={styles.small}>
                    {suggestion.targetReason}
                  </Text>
                </View>
                <View style={styles.helpRight}>
                  <Text style={styles.helpMinutes}>
                    {formatMinutes(suggestion.suggestedMinutes)}
                  </Text>
                  <TouchableOpacity
                    style={styles.assignButton}
                    onPress={() => assignHelp(suggestion)}
                  >
                    <Text style={styles.assignText}>Assign Help</Text>
                  </TouchableOpacity>
                </View>
              </View>
            ))}
        ...
        {activeHelpActions.length > 0 ? (
          <>
            <Text style={styles.sectionTitle}>Active Help</Text>
            {activeHelpActions.map((action) => (
              <View key={action.id} style={styles.activeHelpCard}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.cardTitle}>
                    {employeeName(employees, action.helperEmployeeId)} → {action.toTaskName}
                  </Text>
                  <Text style={styles.small}>
                    {action.fromTaskName
                      ? `From ${action.fromTaskName}`
                      : 'Using spare shift capacity'} · {formatMinutes(action.minutes)}
                  </Text>
                </View>
                <View style={styles.helpActionButtons}>
                  <TouchableOpacity
                    style={styles.helpDoneButton}
                    onPress={() => finishHelp(action)}
                  >
                    <Text style={styles.helpDoneText}>Done</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={styles.helpCancelButton}
                    onPress={() => cancelHelp(action)}
                  >
        ...
        {employeePlans.length === 0 ? (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>No rostered team</Text>
          </View>
        ) : (
          employeePlans.map((plan) => {
            ...
            const freeForHelp = helpResult.helperFreeMinutes[plan.employeeId] || 0;
            ...
            const status = !started
              ...
              : finished
                ...
                : !arrival?.arrived
                  ...
                  : freeForHelp > 0
                    ? `Shift active · ${formatMinutes(freeForHelp)} spare for help`
            ...
            return (
              <View key={plan.employeeId} style={styles.card}>
                <View style={styles.cardTop}>
                  <Text style={styles.cardTitle}>
                    {employeeName(employees, plan.employeeId)}
                  </Text>
                  <Text style={styles.small}>
                    {formatNightMinute(plan.shiftStartMinute)} → {formatNightMinute(plan.shiftFinishMinute)}
                  </Text>
                </View>
                <Text style={freeForHelp > 0 ? styles.goodText : styles.activeText}>
                  {status}
                </Text>
                <Text style={styles.small}>
                  Before load {formatMinutes(plan.preLoadMinutes)} · Load labour {formatMinutes(plan.availableAfterLoadMinutes)} · Allocated {formatMinutes(plan.allocatedMinutes)}
                </Text>
              </View>
            );
          })
        ...
        {taskPlans.length === 0 ? (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>No approved allocation</Text>
            <Text style={styles.small}>Save the final Staff Allocation first.</Text>
          </View>
        ) : (
          taskPlans.map((plan) => {
            const item = findProgress(plan.taskName);
            ...
            const helpNeed = helpResult.helpNeededTasks.find(
              (need) => need.taskName === plan.taskName
            );
            const taskHelpActive = activeHelpActions.filter(
              (action) => action.toTaskName === plan.taskName
            );
              ...
              <View key={plan.taskName} style={styles.card}>
                <View style={styles.cardTop}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.small}>
                      {plan.employeeIds
                        .map((id) => employeeName(employees, id))
                        .join(', ')}
                    </Text>
                  </View>
                  <View style={styles.badgeStack}>
                    ...
                    {helpNeed ? (
                      <Text style={styles.helpBadge}>HELP NEEDED</Text>
                    ) : null}
                  </View>
                </View>
                ...
                <View style={styles.row}>
                  <View style={styles.clockCard}>
                    ...
                    <Text style={styles.clock}>{formatNightMinute(start)}</Text>
                    <Text style={styles.small}>
                      {item?.startMode === 'manual' ? 'MANUAL' : 'AUTO'}
                    </Text>
                  </View>
                  <View style={styles.clockCard}>
                    <Text style={styles.label}>PLAN FINISH</Text>
                    <Text style={styles.clock}>
                      {formatNightMinute(plan.plannedFinishMinute)}
                    </Text>
                    <Text style={styles.small}>
                      {formatMinutes(plan.elapsedMinutes)} elapsed
                    </Text>
                  </View>
                </View>

                <Text style={styles.small}>
                  {plan.staffCount} staff · {formatMinutes(plan.allocatedLabourMinutes)} labour
                </Text>
                <Text
                  style={
                    status === 'Complete'
                ...
                {taskHelpActive.length > 0 ? (
                  <Text style={styles.helpActiveText}>
                    Help active: {taskHelpActive
                      .map((action) => employeeName(employees, action.helperEmployeeId))
                      .join(', ')}
                  </Text>
                ) : null}
                ...
                <View style={styles.row}>
                  <TouchableOpacity
                    ...
                    onPress={() => openStartEditor(plan)}
                  >
                    <Text style={styles.secondaryText}>Adjust Start</Text>
                  </TouchableOpacity>

                  {status !== 'Complete' ? (
                    <TouchableOpacity
                      style={styles.helpButton}
                      onPress={() => setManualHelpTask(plan.taskName)}
                    >
        ...
        <TouchableOpacity
          ...
          onPress={() => router.push('/night-summary')}
        >
          <Text style={styles.summaryText}>Night Summary →</Text>
        </TouchableOpacity>
      </ScrollView>
            ...
            <View style={styles.row}>
              ...
              {editingTask ? (
                <TouchableOpacity
                  ...
                  onPress={() => useAutomaticStart(editingTask)}
                >
                  <Text style={styles.autoText}>Use Auto</Text>
                </TouchableOpacity>
              ) : null}
              <TouchableOpacity
                style={styles.completeButton}
                onPress={saveManualStart}
              >
...
const styles = StyleSheet.create({
  ...
  helpButtonText: { color: '#B54708', fontSize: 10, fontWeight: '800' },
  completeButton: {
    flex: 1,
    backgroundColor: '#168455',
    borderRadius: 10,
    paddingVertical: 11,
    alignItems: 'center',
  },
  ...
  managerHelpTitle: { color: '#B42318', fontSize: 13, fontWeight: '900' },
  managerHelpText: { color: '#912018', fontSize: 9, lineHeight: 14, marginTop: 4 },
  managerHelpItem: { color: '#912018', fontSize: 9, fontWeight: '700', marginTop: 6 },
  helpSuggestionCard: {
  ...
  helpSuggestionTitle: { color: '#101828', fontSize: 12, fontWeight: '800' },
  helpRight: { alignItems: 'flex-end', gap: 7 },
  helpMinutes: { color: '#B54708', fontSize: 15, fontWeight: '900' },
  assignButton: {
  ...
  assignText: { color: '#FFFFFF', fontSize: 8, fontWeight: '900' },
  activeHelpCard: {
  ...
  helpDoneText: { color: '#FFFFFF', fontSize: 8, fontWeight: '900' },
  helpCancelButton: {
    backgroundColor: '#FFFFFF',
    borderRadius: 8,
    paddingHorizontal: 10,
  ...
  helpCancelText: { color: '#667085', fontSize: 8, fontWeight: '800' },
  helpActiveText: { color: '#168455', fontSize: 9, fontWeight: '800', marginTop: 6 },
  summaryButton: {
    backgroundColor: '#101D48',
    borderRadius: 14,
    paddingVertical: 15,
